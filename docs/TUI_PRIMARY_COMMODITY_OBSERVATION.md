...
The current TUI is already partly convenient, but the convenience does not yet have one semantic owner.
...
can already work when the selected Accounts provide one unambiguous default Commodity. This is Account-derived inference, not a Household-wide JPY setting.
...
That makes the common interaction short, but the meaning is duplicated in presentation code instead of coming from canonical Household configuration.

## Desired semantic coordinate
...
The Haskell owner should admit this to the existing typed `Commodity` meaning rather than retaining a presentation string.

The BQN owner should admit the same source coordinate to the same semantic meaning in its native representation.
...
## Shared-source migration constraint

`household.toml` is shared by h-kernel and bqn-ledger.

Current bqn-ledger Household admission is deliberately fail-closed for unsupported TOML sections. Therefore adding `[money]` to canonical data before both engines understand it would make the BQN reader reject the Household.

The safe migration order is:

1. add typed `primary-commodity` admission to h-kernel;
2. add equivalent strict admission to bqn-ledger;
3. prove both readers on fixtures;
4. add `[money] primary-commodity = "JPY"` to canonical `household.toml`;
5. only then replace adapter-level hard-coded `JPY` defaults with the admitted Household coordinate;
6. separately decide where quantity-only entry may fall back from Account defaults to Household primary Commodity.

Do not introduce a hidden compatibility fallback such as "missing means JPY" merely to avoid coordinating the source migration.
